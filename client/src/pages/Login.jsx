...
const Login = () => {
  ...
  const login = async (ev) => {
    try {
      ev.preventDefault();
      const { data } = await axios.post(URI + "/api/v1/auth/login", formData, {
    ...
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400 relative' >
      ...
      <div className='bg-slate-900 p-10 sm:p-12 rounded-3xl shadow-2xl w-full sm:w-96 text-indigo-300 text-sm'>
        ...
        <form onSubmit={login} className='space-y-4'>
          ...
          <p onClick={() => navigate('/reset-password')} className='text-indigo-500 hover:text-indigo-400 transition text-xs text-right cursor-pointer'>
            Forgot Password?
          </p>
        ...
        <Link to="/register" className='text-center block text-xs mt-6 text-indigo-300 hover:text-indigo-200 transition'>
          Don't have an account? <span className='underline text-indigo-400'>Sign up</span>
        </Link>
      </div>
    </div>
  )
